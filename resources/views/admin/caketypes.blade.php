        ...
        <div class="modal-dialog modal-xl modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" action="cake-types/add" class="change-form" enctype="multipart/form-data">
                        @csrf
...
<div class="table-responsive">
    <table class="table table-bordered text-center"  id="data_types">
        <thead>
        <tr>
        ...
        <tbody id="search-body">
            ...
            <tr>
                <td class="btn-gr d-none"><input type="checkbox" class="check" name="delete[]" value="{{ $types->id }}"></td>
                <td>{{ $types->title }}</td>
                <td>{{ $types->created_at }}</td>
                <td>
                    <div class="btn-group-sm btn-func">
                        <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#editModal{{$types->id}}">
                            <span class="fas fa-edit"></span>
                        </button>
                        <a href="cake-types/delete/{{$types->id}}" class="btn btn-primary" onclick="return confirm('Bạn có chắc chắn muốn xóa?')">
                            <span class="fas fa-trash-alt"></span>
                        </a>
                    </div>
                </td>
                {{-- form edit --}}
                <div class="modal fade" id="editModal{{$types->id}}">
                    <div class="modal-dialog modal-xl modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header">
                            ...
                            <div class="modal-body">
                                <form method="POST" action="cake-types/edit/{{$types->id}}" class="change-form" enctype="multipart/form-data">
                                    @csrf
                                    <label for="" class="required control-label">@lang('modules.levels.name')</label>
                                    <input class="form-control" value="{{$types->title}}" name="title" required>
